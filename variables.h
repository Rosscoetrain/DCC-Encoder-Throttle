...
int LocoAddress[MAXLOCOS] = {5, 13, 14, 15, 16, 99};
int LocoDirection[MAXLOCOS] = {1, 1, 1, 1, 1, 1};
int LocoSpeed[MAXLOCOS] = {0, 0, 0, 0, 0, 0};
...
String LocoDefs[MAXLOCOS] = {"Marklin 3000    ",
                             "Marklin 3056    ",
                             "Marklin 3005    ",
                             "Marklin 3048  1 ",
                             "Marklin 3048  2 ",
...
byte rowPins[ROWS] = {5, 6, 7, 8 };       //{8,7,6,5 };   //connect to the row pinouts of the keypad
byte colPins[COLS] = {9, 10, 11, 12};     // {11,10,9};   //connect to the column pinouts of the keypad
Keypad keypad = Keypad( makeKeymap(keys), rowPins, colPins, ROWS, COLS );
#endif
...
SoftwareSerial lcd(A0, A1);          // Arduino SS_RX = pin A0 (unused), Arduino SS_TX = pin A1
                                     /*
                                      *   Using analog pin A0 for SS_RX as future update will allow reading of loco roster from DCC-EX.
                                      *   Using analog pin A1 for SS_TX as only spare digital pin is 12 which is used for 4x4 keypad
                                      */
#endif
